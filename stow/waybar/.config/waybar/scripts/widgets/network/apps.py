...
def app_tooltip(interface):
    try:
        ...
        if active:
            ...
            lines.extend(
                f"{a['name']}: ↓ {human_rate(a['down_rate'])}  ↑ {human_rate(a['up_rate'])}"
                for a in current
            )
            if not current:
            ...
            lines.append(
                "Collecting…"
                if fresh and data.get("status") == "collecting"
                else "Live monitoring unavailable"
            )
        ...
        lines.extend(
            f"{a['name']}: ↓ {human_bytes(a['received'])}  ↑ {human_bytes(a['sent'])}"
            for a in totals
        )
        if not totals:
            lines.append("No recorded traffic yet")
        lines += [
            "Approximate TCP/UDP totals on the monitored default interface(s)",
