...
from ..common import clamp, classes, perf_text, push_history, run, use_compact_perf_text
...
def network_module(state):
    ...
    if not iface:
        ...
        return {
            "text": "󰖪 off",
            "tooltip": "\n".join(["No active network interface", *app_tooltip(None)]),
    ...
    try:
        ...
        return {
            "text": "󰖪 off",
            "tooltip": f"{iface}: traffic counters unavailable",
            "class": classes("metric", "muted"),
        }
    now = time.monotonic()
    ...
    lines += [
        f"Gateway: {route.get('gateway', 'Unavailable')}",
        f"Down: {human_rate(down)}",
        f"Up: {human_rate(up)}",
        f"Received: {human_bytes(rx)}",
        f"Sent: {human_bytes(tx)}",
