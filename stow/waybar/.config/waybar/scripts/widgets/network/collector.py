...
class Accounting:
    ...
    def update(self, rows, now):
        ...
        for identity, counters in rows.items():
            ...
            delta = [new - prev if new >= prev else new for new, prev in zip(counters, old)]
            ...
            app = self.data['apps'].setdefault(key, {
                'name': name, 'uid': uid if pid > 0 else -1,
                'sent': 0, 'received': 0, 'up_rate': 0, 'down_rate': 0,
            })
            app['sent'] += delta[0]
...
def default_interface():
    for family in ([], ['-6']):
        result = subprocess.run(['/usr/bin/ip', '-j', *family, 'route', 'show', 'default'],
...
def capture(accounting, interface):
    ...
    with subprocess.Popen(command, stdout=subprocess.PIPE, text=True,
                          encoding='utf-8', errors='replace', env={**os.environ, 'LC_ALL': 'C'}) as process:
        events = queue.Queue()
