...
class WidgetTests(unittest.TestCase):
    ...
    def test_concurrent_updates(self):
        ...
        workers = [context.Process(target=increment, args=(str(self.cache), str(self.legacy))) for _ in range(4)]
    ...
    def test_registry_and_escape(self):
        for widget, handler in (loader.MODULES | loader.ACTIONS).values():
            self.assertTrue(callable(getattr(importlib.import_module(f'widgets.{widget}'), handler)))
    ...
    def test_process_ranking_and_pid_reuse(self):
        ...
        current = {'1': {'ticks': 300, 'start': '5', 'rss': 1000, 'name': 'worker'},
    ...
    def test_gpu_partial_and_missing_data(self):
        ...
        with patch.object(gpu, 'run', return_value=result), patch.object(gpu, 'use_compact_perf_text', return_value=True):
            tooltip = gpu.gpu_module({})['tooltip']
            self.assertIn('25.0%', tooltip)
            self.assertIn('Power draw: Unavailable', tooltip)
        with patch.object(gpu, 'run', return_value=subprocess.CompletedProcess([], 1, '')), patch.object(gpu, 'use_compact_perf_text', return_value=True):
            self.assertIn('unavailable', gpu.gpu_module({})['tooltip'])

    def test_memory_swap_and_cache(self):
        fixture = 'MemTotal: 8192 kB\nMemAvailable: 2048 kB\nCached: 1000 kB\nSReclaimable: 100 kB\nShmem: 50 kB\nSwapTotal: 2048 kB\nSwapFree: 1024 kB\n'
        with patch.object(Path, 'read_text', return_value=fixture), patch.object(memory, 'snapshot', return_value={}), patch.object(memory, 'use_compact_perf_text', return_value=True):
    ...
    def test_cpu_delta_and_first_sample(self):
        ...
        with patch.object(Path, 'read_text', read), patch.object(cpu, 'snapshot', return_value={}), patch.object(cpu, 'temperature', return_value='Unavailable'), patch.object(cpu, 'use_compact_perf_text', return_value=True):
    ...
    def test_monitor_compact_layout(self):
        ...
        with patch('widgets.common.run', return_value=result), patch('widgets.common.PERF_PRIMARY_MONITOR', 'DP-1'):
