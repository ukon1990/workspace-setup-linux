...
Linux bootstrap (also reachable via ./bootstrap.sh).
...
if [[ $RUN_PACKAGES -eq 1 ]]; then
  if [[ $AUTO_YES -eq 1 ]] || ask_yes_no 'Install repo packages now (common -> Hyprland -> apps -> AUR)?'; then
    run_step 'Installing repo packages' env DRY_RUN="$DRY_RUN" "$ROOT/scripts/install-packages.sh"
  fi
fi
...
if [[ $RUN_LINK -eq 1 ]]; then
  run_step 'Linking configs into your home directory' env DRY_RUN="$DRY_RUN" "$ROOT/scripts/link-configs.sh"
fi
