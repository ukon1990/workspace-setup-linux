...
def gpu_module(state):
    result = run(['nvidia-smi', '--query-gpu=name,utilization.gpu,memory.used,memory.total,temperature.gpu,power.draw,power.limit', '--format=csv,noheader,nounits'])
    ...
    if not rows:
        ...
        return {'text': perf_text('󰢮', 'n/a', history, compact),
    ...
    for name, util, used, total, temp, power, limit in rows:
        if lines:
            lines.append('')
        lines.append(name)
        for label, raw, unit in [('GPU usage', util, '%'), ('Temperature', temp, ' °C'),
                                 ('Power draw', power, ' W'), ('Power limit', limit, ' W')]:
            value = number(raw)
            lines.append(f'{label}: {value:g}{unit}' if value is not None else f'{label}: Unavailable')
        mem_used, mem_total = number(used), number(total)
        lines.append(f'VRAM: {mem_used:g} / {mem_total:g} MiB ({mem_used / mem_total * 100:.1f}%)'
                     if mem_used is not None and mem_total and mem_total > 0 else 'VRAM: Unavailable')
    usage = number(rows[0][1])
    history = push_history(state, 'gpu_history', usage or 0)
    return {'text': perf_text('󰢮', f'{usage:.0f}%' if usage is not None else 'n/a', history, compact),
