...
from .common import classes, clamp, perf_text, push_history, use_compact_perf_text
...
def cpu_module(state):
    ...
    lines = [model, f'CPU usage: {usage:.1f}%' if previous else 'CPU usage: Collecting…',
             f'Cores / threads: {len(cores) if cores else "Unknown"} / {len(records)}',
             'Load (1 / 5 / 15 min): ' + ' / '.join(f'{v:.2f}' for v in os.getloadavg()),
             f'Temperature: {temperature()}', '', 'Top CPU processes (100% = one logical CPU):']
    now = time.monotonic()
    current = snapshot()
    ...
    return {'text': perf_text('', f'{usage:.0f}%', history, compact),
