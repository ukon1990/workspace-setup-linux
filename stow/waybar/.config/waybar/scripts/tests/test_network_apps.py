...
from widgets.network import apps as network_apps
from widgets.network import collector


class AccountingTests(unittest.TestCase):
    def test_trace_parse_and_bad_input(self):
        self.assertEqual(
            collector.parse_row("/opt/my browser/browser/42/1000\t123\t456\n"),
            (("/opt/my browser/browser", 42, 1000), (123, 456)),
        )
        for line in [
            "Refreshing:",
            "Unknown connection: 1.2.3.4",
            "x/1/2\tnan\t3",
            "x/1/2\t-1\t3",
            "x/1/2\tinf\t3",
        ]:
    ...
    def test_unknown_traffic(self):
        ...
        self.assertEqual(
            next(iter(accounting.data["apps"].values()))["name"], "Unattributed traffic"
        )
    ...
    def test_default_route_selection(self):
        ...
        with patch.object(
            ...
            return_value=CompletedProcess(
                [], 0, '[{"dev":"wlan0","metric":600},{"dev":"eth0","metric":100}]'
            ),
        ):
